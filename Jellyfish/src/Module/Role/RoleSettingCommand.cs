...
public class RoleSettingCommand : GuildMessageCommand
{
    ...
    private async Task ListPermissions(SocketTextChannel channel)
    {
        ...
        var roles = (
            from role in dbCtx.UserRoles.Include(e => e.CommandPermissions).AsNoTracking()
            orderby role.KookId
    ...
    private async Task<(
        string commandName,
        string guildRoleName,
        uint guildRoleId,
        UserCommandPermission? record
    )?> ExtractPermissionBindingParameters(
        string rawArgs,
        SocketTextChannel channel,
    ...
    )
    {
        ...
        {
            await channel.SendErrorCardAsync(
                "角色不存在，您可以发送 `!权限 服务器角色` 列出全部角色名称（或查看当前服务器配置）",
                true
            );
            return null;
        }
    ...
    private async Task<bool> BindingPermission(string rawArgs, SocketTextChannel channel)
    {
        ...
        AppCaches.Permissions.AddOrUpdate(
            $"{channel.Guild.Id}_{commandName}",
            [guildRoleId],
            (_, v) =>
            {
                v.Add(role.KookId);
        ...
        await channel.SendSuccessCardAsync(
            $"权限绑定成功！角色 {guildRoleName} 可以执行 {commandName}",
            false
        );
        return true;
    }
    ...
    private async Task<bool> UnBindingPermission(string rawArgs, SocketTextChannel channel)
    {
        ...
        else
        {
            ...
            await channel.SendSuccessCardAsync(
                $"权限解绑成功！角色 {guildRoleName} 已无法使用 {commandName}",
                false
            );
        }
    ...
    /// <param name="channel">Current channel</param>
    /// <returns>Is command success or not</returns>
    private async Task<bool> SetDefaultManagerAccountsAndRoles(
        string rawArgs,
        SocketTextChannel channel
    )
    {
        ...
        foreach (Match match in Regexs.MatchUserMention().Matches(rawArgs))
        {
            ...
            {
                await channel.SendErrorCardAsync(
                    "您应该使用 @ 来指定管理员用户，被指定的用户在 Kook APP 中显示为蓝色文字",
                    true
                );
                return false;
            }
        ...
        foreach (Match match in Regexs.MatchRoleMention().Matches(rawArgs))
        {
            ...
            {
                await channel.SendErrorCardAsync(
                    "您应该使用 @ 来指定管理员角色，被指定的用户在 Kook APP 中显示为蓝色文字",
                    true
                );
                return false;
            }
