...
public class WeiboScanAndPushJob(
    ...
    ILogger<WeiboScanAndPushJob> log
) : IAsyncJob
{
    public async Task ExecuteAsync()
    {
        ...
        )
        {
            ...
            if (counts > 0)
                log.LogInformation("微博用户{UID}已推送完毕，共{Count}条", uid, counts);
            else
                log.LogInformation("未发现新微博，跳过微博用户{UID}", uid);
        }

        log.LogInformation("微博推送任务完成");
    }

    private async Task<Dictionary<bool, List<WeiboItem>>> GetChangedWeiboAsync(
    ...
    )
    {
        ...
        var existed = dbCtx
            .WeiboCrawlHistories.Where(it => it.Uid == uid && ids.Contains(it.Mid))
            .ToList();
        ...
        if (newWeiboList.IsNotEmpty())
        {
            ...
            var histories = newWeiboList
                .Select(w => new WeiboCrawlHistory(
                    uid,
                    w.Md5,
                    w.Username,
                    w.Content,
        ...
        // Get changed weibo
        var changeWeiboList = weiboList
            .Where(it => existed.Exists(e => e.Mid == it.Mid && e.Hash != it.Md5))
            .ToList();
        if (!changeWeiboList.IsNotEmpty())
            return result;
    ...
    private async Task<int> ScanAndPushAsync(DatabaseContext dbCtx, string uid)
    {
        ...
        {
            var instances = (
                from i in dbCtx.WeiboPushInstances.Include(i => i.Config).AsNoTracking()
        ...
        // Edit weibo
        if (
            !changedWeibo.TryGetValue(false, out var changedWeiboList)
            || !changedWeiboList.IsNotNullOrEmpty()
        )
            return changeCount;
    ...
    private async Task PushToEachChannelAsync(
        DatabaseContext dbCtx,
        List<WeiboPushInstance> instances,
        List<WeiboItem> weiboList
    )
    {
        foreach (var instance in instances)
        {
            var channel = kook.GetGuild(instance.Config.GuildId)
                ?.GetTextChannel(instance.ChannelId);
            if (channel == null)
                continue;
            ...
            foreach (var weibo in weiboList)
            {
                try
                {
                    ...
                    dbCtx.WeiboPushHistories.Add(
                        new WeiboPushHistory(instance.Id, weibo.Md5, weibo.Mid, message.Id)
                    );
                }
                catch (Exception e)
                {
                    log.LogError(
                        e,
                        "推送微博的过程中发生未知错误，用户：{Username}，内容：{Content}，MD5：{MD5}",
                        weibo.Username,
                        weibo.Content[..(weibo.Content.Length > 20 ? 20 : weibo.Content.Length)],
                        weibo.Md5
                    );
                }
            }
    ...
    private async Task UpdatePrevWeiboAsync(DatabaseContext dbCtx, List<WeiboItem> weiboList)
    {
        ...
        {
            var histories = dbCtx
                .WeiboPushHistories.Include(h => h.Instance)
