...
) : UserConnectEventCommand
{
    ...
    )
    {
        ...
        var room = dbCtx
            .TpRoomInstances.Include(e => e.TmpTextChannel)
            .FirstOrDefault(room =>
                room.VoiceChannelId == channel.Id && room.GuildId == channel.Guild.Id
            );

        if (room?.TmpTextChannel is null)
        ...
        if (channel.HasPassword)
        {
            ...
            await restTextChannel.OverrideUserPermissionAsync(
                user.Value!,
                p => p.Modify(viewChannel: PermValue.Allow, mentionEveryone: PermValue.Allow)
            );
            log.LogInformation(
                "加入语音房间 {TpRoomName} 的用户：{Name}:{Id}，已获得文字房间 {TextRoomName} 的访问权限",
                room.RoomName,
                user.Value!.DisplayName(),
                user.Id,
                restTextChannel.Name
            );
        ...
        else
        {
            await restTextChannel.OverrideUserPermissionAsync(
                user.Value!,
                p => p.Modify(sendMessages: PermValue.Allow)
            );
            log.LogInformation(
                "加入语音房间 {TpRoomName} 的用户：{Name}:{Id}，已在文字房间 {TextRoomName} 中被标记",
                room.RoomName,
                user.Value!.DisplayName(),
                user.Id,
                restTextChannel.Name
