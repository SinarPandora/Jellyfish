...
public class TeamPlayTeammateJoinCommand : UserConnectEventCommand
{
        ...
        DateTimeOffset joinAt)
    {
        ...
        var textChannel = channel.Guild.GetTextChannel(tmpInstance.ChannelId);
        if (textChannel == null)
        {
        ...
        {
            await textChannel.OverrideUserPermission(user.Value, p => p.Modify(viewChannel: PermValue.Allow));
            _log.LogInformation("加入组队房间 {TpRoomName} 的用户：{Name}:{Id}，已获得文字房间 {TextRoomName} 的访问权限",
                room.RoomName, user.Value.DisplayName(), user.Id, textChannel.Name
            );
            await textChannel.SendSuccessCardAsync($"欢迎 {MentionUtils.KMarkdownMentionUser(user.Id)} 加入组队房间！", false);
        }
