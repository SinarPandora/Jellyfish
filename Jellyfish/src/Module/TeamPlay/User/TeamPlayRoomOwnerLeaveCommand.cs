...
/// </summary>
public class TeamPlayRoomOwnerLeaveCommand(
    ILogger<TeamPlayRoomOwnerLeaveCommand> log,
...
) : UserDisconnectEventCommand
{
    ...
    )
    {
        ...
        var room = (
            from instance in dbCtx.TpRoomInstances
            where instance.GuildId == channel.Guild.Id && instance.VoiceChannelId == channel.Id
            select instance
        ).FirstOrDefault();

        if (room is null || room.OwnerId != user.Value!.Id)
