...
/// </summary>
public class TeamPlayButtonActionEntry(
    TeamPlayManageService service,
    ILogger<TeamPlayButtonActionEntry> log
) : ButtonActionCommand
{
    ...
    )
    {
        ...
        {
            log.LogInformation(
                "已阻止用户 {Username} 操作不属于他的卡片按钮",
                user.Value!.Username
            );
            return CommandResult.Done;
        }

        await service.BindingVoiceChannel(args[1], user, channel);
        return CommandResult.Done;
    }
}
