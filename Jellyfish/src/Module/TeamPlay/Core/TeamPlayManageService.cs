...
/// </summary>
public class TeamPlayManageService(ILogger<TeamPlayManageService> log, DbContextProvider dbProvider)
{
    ...
    public async Task<bool> SendBindingWizard(
        SocketUser user,
    ...
    )
    {
        ...
        var names = dbCtx
            .TpConfigs.EnabledInGuild(channel.Guild)
            .AsNoTracking()
            .Select(c => c.Name)
            .ToHashSet();
        ...
        cardBuilder
            ...
            .AddModule<SectionModuleBuilder>(s =>
            {
                s.WithText("当绑定了一个语音入口频道或文字入口频道后，配置就可以使用啦", true);
            })
            .WithSize(CardSize.Large);

        await channel.SendCardSafeAsync(cardBuilder.Build());
        log.LogInformation("已发送绑定向导，目标类型：{Name}", name);
    ...
    /// <param name="user">Action user</param>
    /// <param name="channel">Current channel</param>
    public async Task BindingVoiceChannel(
        string name,
    ...
    )
    {
        ...
        {
            log.LogInformation(
                "已检测到语音频道：{VoiceChannelName}：{VoiceChannelId}",
                voiceChannel.Name,
                voiceChannel.Id
            );
            await channel.SendInfoCardAsync(
                $"检测到您加入了频道：{voiceChannel.Name}，正在绑定...",
                false
            );
            ...
            {
                config = new TpConfig(name, channel.Guild.Id) { VoiceChannelId = voiceChannel.Id };
            ...
            log.LogInformation(
                "成功绑定 {Name} 到 {VoiceChannelName}：{VoiceChannelId}，ID：{ConfigId}",
                name,
                voiceChannel.Name,
                voiceChannel.Id,
                config.Id
    ...
    public async Task<bool> BindingTextChannel(SocketTextChannel channel, string rawArgs)
    {
        ...
        await channel.SendSuccessCardAsync(
            $"绑定成功！{MentionUtils.KMarkdownMentionChannel(bindingChannelId)}已与组队配置 {configName} 绑定",
            false
        );

        log.LogInformation(
            "成功绑定 {Name} 到 {ChannelName}：{ChannelId}，ID：{ConfigId}",
            configName,
            bindingChannelName,
            bindingChannelId,
            config.Id
    ...
    public async Task<bool> RemoveConfig(SocketTextChannel channel, string name)
    {
        ...
        {
            log.LogWarning(
                "组队配置缓存缺失，可能是一个 bug。配置键：{GuildId}_{Name}",
                channel.Guild.Id,
                name
            );
        }
    ...
    private static async Task SendFurtherConfigIntroMessage(
            ...
            new CardBuilder()
                .AddModule<SectionModuleBuilder>(s =>
                {
                    s.WithText(
                        ...
                        默认人数：{(
                            config.DefaultMemberLimit == 0
                                ? "无限制"
                                : config.DefaultMemberLimit.ToString()
                        )}
                        语音质量：当前频道默认最高质量
                        ---
                        **设置房间名格式：**
    ...
    public async Task<bool> SetRoomPattern(SocketTextChannel channel, string msg)
    {
        ...
        {
            await channel.SendErrorCardAsync(
                $"参数不足！举例：`!组队 房间名格式 上分 【上分】{UserInjectKeyword}`",
                true
            );
            return false;
        }
    ...
    public async Task<bool> SetDefaultMemberCount(SocketTextChannel channel, string msg)
    {
        ...
        {
            await channel.SendErrorCardAsync(
                "指令格式错误！人数请使用阿拉伯数字正数，若要设置无限人数房间，请使用 0，举例：`!组队 默认人数 上分 4`",
                true
            );
            return false;
        }
    ...
    /// <returns>Should keep the user message or not</returns>
    public async Task<bool> SetCategoryChannel(
        SocketTextChannel channel,
        string rawArgs,
    ...
    )
    {
        ...
        if (channelType == AdditionChannelType.CreationNotify)
        {
            ...
            AppCaches.TeamPlayConfigs[$"{channel.Guild.Id}_{configName}"].CreationNotifyChannelId =
                textChannelId;
        }
        else
        {
            ...
            {
                await channel.SendErrorCardAsync(
                    "指定的文字频道不属于任何分组，为确保频道列表简洁，请重新选择一个带有分组的文字频道",
                    true
                );
                return false;
            }
            ...
            if (channelType == AdditionChannelType.TmpTextCategoryInto)
            {
                ...
                AppCaches.TeamPlayConfigs[$"{channel.Guild.Id}_{configName}"].TextCategoryId =
                    categoryId;
            }
            else
            {
                tpConfig.VoiceCategoryId = categoryId;
                AppCaches.TeamPlayConfigs[$"{channel.Guild.Id}_{configName}"].VoiceCategoryId =
                    categoryId;
            }
        }
    ...
    /// <returns>Should keep the user message or not</returns>
    public async Task<bool> SetTmpTextChannelEnabled(
        SocketTextChannel channel,
        string configName,
        bool enabled
    )
    {
        ...
        dbCtx.SaveChanges();
        AppCaches.TeamPlayConfigs[$"{channel.Guild.Id}_{configName}"].EnableTmpTextChannel =
            enabled;

        await channel.SendSuccessCardAsync(
            $"已{(enabled ? "开启" : "关闭")}临时文字频道功能",
            false
        );
        return true;
    }
