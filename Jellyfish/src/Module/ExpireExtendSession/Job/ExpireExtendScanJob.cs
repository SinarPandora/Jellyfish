...
) : IAsyncJob
{
    ...
    public async Task ExecuteAsync()
    {
        ...
        foreach (var session in dbCtx.ExpireExtendSessions.ToArray())
        {
            ...
            catch (Exception e)
            {
                log.LogError(
                    e,
                    "延长会话处理失败，会话 ID：{SessionId}，会话目标：{TargetType}，目标 ID：{TargetId}",
                    session.Id,
                    session.TargetType,
                    session.TargetId
                );
    ...
    private async Task HandleTmpTextChannelExpire(
        Data.ExpireExtendSession session,
        DateTimeOffset now,
        DatabaseContext dbCtx
    )
    {
        var handled = false;
        var instance = dbCtx.TmpTextChannels.FirstOrDefault(i => i.Id == session.TargetId);
        if (instance is not null)
        {
            ...
            if (textChannel is not null)
            {
                ...
                if (
                    messages.IsNotEmpty()
                    && messages.First().Timestamp.Add(session.TimeUnit.ToTimeSpan(session.Value))
                        > now
                )
                    return;

                log.LogInformation(
                    "监测到临时文字房间到期，开始清理房间：{Name}#{Id}",
                    textChannel.Name,
                    textChannel.Id
                );
                await textChannel.DeleteAsync();
                log.LogInformation(
                    "在服务器中的房间实例已被清理，数据库中的房间数据将由临时房间清理任务清理"
                );
                handled = true;
            }
        }
