...
) : IAsyncJob
{
    public async Task ExecuteAsync()
    {
        try
        {
            ...
            var scope = dbCtx
                .ClockInCardInstances.Include(i => i.Config)
            ...
            foreach (var group in scope)
            {
                ...
                var todayClockInCount = dbCtx.ClockInHistories.Count(h =>
                    h.ConfigId == group.First().ConfigId && h.CreateTime >= today
                );
                if (todayClockInCount > 0)
                {
                    var top3Usernames = dbCtx
                        .ClockInHistories.Include(h => h.UserStatus)
                    ...
                    lastHistory = dbCtx
                        .ClockInHistories.Where(h => h.CreateTime >= today)
                ...
                foreach (var instance in group)
                {
                    ...
                    {
                        log.LogInformation(
                            "{Reason}，重新发送消息，频道：{ChannelName}#{ChannelId}，服务器：{GuildName}",
                            resendReason,
                            channel.Name,
                            channel.Id,
                            guild.Name
                        ...
                        instance.MessageId = await ClockInManageService.SendCardToCurrentChannel(
                            channel,
                            instance.Config,
                            appendData
                        );
                    ...
                    if (lastMessage.IsEmpty())
                    {
                        ...
                        log.LogInformation(
                            "打卡消息被删除，重新发送消息，频道：{ChannelName}#{ChannelId}，服务器：{GuildName}",
                            channel.Name,
                            channel.Id,
                            guild.Name
                        );
                    ...
                    {
                        log.LogInformation(
                            "打卡消息不在频道最底部，重新发送消息，频道：{ChannelName}#{ChannelId}，服务器：{GuildName}",
                            channel.Name,
                            channel.Id,
                            guild.Name
                        );
