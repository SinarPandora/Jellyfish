...
public class ClockInStageManageCommand : GuildMessageCommand
{
    ...
    public ClockInStageManageCommand(ClockInStageManageService service) : base(true)
    {
        ...
        HelpMessage = HelpMessageHelper.ForMessageCommand(this,
            ...
            ---
            1. `[阶段ID]`：显示该阶段详细信息
            2. `[阶段ID] 开始日期 [日期（年-月-日）]`：修改开始日期
            3. `[阶段ID] 结束日期 [日期（年-月-日）]`：修改结束日期（包含当天）
            4. `[阶段ID] 达标天数 [大于 0 整数]`：修改达标天数
            5. `[阶段ID] 合格消息 [消息内容]`：设置合格时向用户发送的消息
            6. `[阶段ID] 给予身份 [@身份引用]`：设置合格时颁发给用户的 Kook 角色
            7. `[阶段ID] 允许中断天数 [大于或等于 0 的整数]`：修改允许中断天数，设置为 0 时不允许中断打卡
            8. `[阶段ID] 禁用`：禁用该阶段
            9. `[阶段ID] 启用`：启用该阶段
